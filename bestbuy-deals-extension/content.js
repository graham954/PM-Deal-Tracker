/*
 * Best Buy Member Deals Scraper — content script
 *
...
/* -------- selector config (the only Best Buy-specific bit) -------- */
// Card containers are tried in order; first one that matches >3 elements wins.
// If Best Buy changes their markup, only this list needs updating.
...
function findCards() {
  // Best Buy "brix" search results: each product is <li data-testid="<skuId>">.
...
function parseCard(el) {
  ...
  // prices via the labels Best Buy renders on each card
...
// Best Buy lazy-renders product cards as you scroll (and may unload off-screen
